# PRD: Claude Code 聊天室插件(nexscope)—— H2A2A2H 协作管道
...
- **P0** 插件通过 `/nexscope:start -n <name>` 显式加入,`/nexscope:stop` 显式离开(**不自动常驻**)。
...
| 角色 | 描述 |
|---|---|
| **Human(房间成员)** | 真人,通过 `/nexscope:start` 加入、`/nexscope:stop` 离开 |
...
**S2 A 让自己 Claude 代发**
Alice 说"让我的 Claude 整理当前 PR 的要点发给 bob",A 的 Claude 生成草稿(role=userAgent),Alice 再 `/nexscope:append` 追加一句自己的话(role=user),一起发给 `@bob`。

**S3 群广播**
Alice 发 `准备下午 release,大家注意 freeze`(无 @,广播)。所有在线成员的 Claude 都收到;因为无点名,默认**不主动回应**,仅展示给各自 human。

**S4 Presence 感知**
Carol `/nexscope:start -n carol` 加入,Alice 和 Bob 的 Claude 各自收到一行 `[presence] carol joined`;Alice 可直接 `@carol hi`。Carol 之后 `/nexscope:stop` → 各客户端 Claude 收到 `[presence] carol left`。
...
| **Username** | 房间内唯一的用户名(如 `alice`、`bob`、`nexscope`)。 |
...
| 模块 | 内容 |
|---|---|
| **插件 `nexscope`** | skill: `start` / `stop` / `say` / `who` / `inbox` / `accept` / `reject` / `append` / `mode` / `history`;后台 monitor(仅在 session 活动时运行) |
| **中继 `nexscope-relay`** | Node + ws,单房间,维护 username 集合、广播 presence、按 @mention 分发 |
...
| **Mode 切换** | 本地 per-session;可通过 env `NEXSCOPE_MODE` 设默认值 |
...
### US-1 显式加入 / 离开
**As** 任意成员,**I want** 用 `/nexscope:start -n <name>` 进房、`/nexscope:stop` 出房,**so that** 我能清楚控制何时接收/不接收消息。
...
- `/nexscope:start -n <username> [--room=<id>] [--mode=manual|auto]` 或 bin `nexscope-start -n <name>`
...
- Role 默认值规则:
  ...
  - Human 直接跑 bin `nexscope-say` → `user`
  - 显式 `--role` 覆盖
...
- 文件: `~/.claude/plugin-data/nexscope/inbox.jsonl`
- 条目: `{threadId, from, role, mentions, text, ts, status:"pending"}`
- Skill:
  - `/nexscope:inbox` — 列出 pending + 最近归档的
  - `/nexscope:accept <threadId> [extra]` — 标 accepted,Claude 把该 thread 所有消息合并作为本轮新的 user 输入执行;可追加 extra 指令
  - `/nexscope:reject <threadId> [reason]` — 标 rejected,向原发送方回发一条 role=user 的拒绝消息(含 reason)
- 超过 24h pending 自动归 expired。

### FR-6 Auto 模式
- `/nexscope:mode auto|manual` 切换本 session 模式;`env NEXSCOPE_MODE` 可设默认。
- Auto 模式规则:
  ...
  5. 人类任何时候可 Ctrl+C 打断当前自动回复,或 `/nexscope:mode manual` 立即退出 auto。

### FR-7 人类直达 / 追加
- `/nexscope:append <threadId> <text>` 向 thread 追加一段 role=user 的消息(本 human 亲自说)。
- bin `nexscope-say --role=user ...` 允许 human 不经 Claude 直接发言。

### FR-8 Who
- `/nexscope:who` 列出当前在线用户(本地缓存,由 presence 维护)。

### FR-9 History(本地)
- `/nexscope:history [--limit=N]` 列出本地收到的最近 N 条消息(不查 relay,仅本地内存/文件)。

### FR-10 身份与鉴权 / 配置来源
- **连接信息来自插件配置文件**(不走 env):
  - 路径:`~/.claude/plugin-data/nexscope/config.json`(与插件目录解耦,升级插件不覆盖用户数据)
  - 首次启动若该路径不存在,自动从插件根的 `config.example.json` 复制过去,并打印提示 `已初始化 ~/.claude/plugin-data/nexscope/config.json,请填入 relayUrl 和 token 后重新运行`
...
- **Env 仅作覆盖**(可选):`NEXSCOPE_RELAY_URL` / `NEXSCOPE_TOKEN` / `NEXSCOPE_DEFAULT_NAME` / `NEXSCOPE_MODE` / `NEXSCOPE_HOP_LIMIT`。存在时优先于 config.json。
...
```
/nexscope:start  -n <name> [--mode=manual|auto]   加入聊天室
/nexscope:stop                                    离开聊天室
/nexscope:say    [@u1 @u2 ...] [--role=...] [--thread=...] <text>  发消息
/nexscope:who                                     列出在线用户
/nexscope:inbox                                   (manual)查看待审批
/nexscope:accept <threadId> [extra]               (manual)批准执行
/nexscope:reject <threadId> [reason]              (manual)拒绝并回发
/nexscope:append <threadId> <text>                追加一段 role=user 消息
/nexscope:mode   auto|manual                      切换模式
/nexscope:history [--limit=N]                     查看本地历史
```

### 8.6 插件配置文件(首选)

**路径**: `~/.claude/plugin-data/nexscope/config.json`(**非**插件根目录——与插件代码分离,升级/重装不会覆盖用户配置)
...
| `defaultName` | 否 | `/nexscope:start` 未传 `-n` 时使用;用户安装插件后通常会改 |
...
- 用户真实配置存于 `~/.claude/plugin-data/nexscope/config.json`(插件自身目录仅含 example,可 commit)
- 插件启动时若 `~/.claude/plugin-data/nexscope/config.json` 不存在 → 自动从插件根 `config.example.json` 复制过去并 `chmod 0600`,然后中止启动并提示用户填写
...
| # | 场景 | 预期 |
|---|---|---|
| AC-1 | Alice `/nexscope:start -n alice`,Bob `/nexscope:start -n bob` | 两人加入成功;Alice 的 Claude 看到 `[presence] bob joined`(或 snapshot) |
| AC-2 | Alice `/nexscope:start -n bob`(bob 已在线) | 连接失败,提示 `username "bob" taken` |
| AC-3 | Alice `/nexscope:say @bob 任务...`(role=userAgent) | Bob 会话中出现 `[userAgent1 alice → @me] ...`;Alice 侧 ack delivered=[bob] |
| AC-4 | Alice `/nexscope:say hello all`(无 @) | Bob、其他在线成员都看到 `[userAgent1 alice → @all] hello all` |
| AC-5 | Manual 模式下 Alice @bob 发任务 | Bob 消息进 inbox,`/nexscope:inbox` 可见,`/nexscope:accept` 后 Claude 按指令执行 |
...
| AC-9 | Bob `/nexscope:stop` | relay 广播 leave;Alice 的 Claude 看到 `[presence] bob left`;Bob 再也收不到消息 |
| AC-10 | Bob 重启 Claude Code(未 stop) | 心跳超时后 relay 广播 leave;Bob 重启完 `/nexscope:start` 恢复 |
| AC-11 | relay 重启 | 所有客户端断线重连;重连后自动重入房间(用 last username);若冲突提示人类 |
| AC-12 | Alice 用 bin 直接发 `nexscope-say --role=user @bob ...` | Bob 看到 `[user1 alice → @me] ...`(人类 role) |
| AC-13 | Alice 的 Claude 生成草稿后 Alice `/nexscope:append` | 同 threadId 两条消息(userAgent + user)按序到达 Bob |
...
| **Q-5** | `/nexscope:stop` 后 inbox 里未处理的消息如何处理? | 建议保留,下次 start 后仍在 inbox;待确认 |
...
| **Q-10** | `/nexscope:accept` 批准后,Claude 是否需要公开回"已执行"?还是完成后回一条消息? | 建议完成后自动发一条 `userAgent` 回 thread;待确认 |

> **已定(v0.3.1 拍板)**:
> - 配置文件路径 = `~/.claude/plugin-data/nexscope/config.json`(与插件目录解耦)
...
2. 确认插件名就叫 `nexscope`(插件目录名、skill 命名空间),skill 调用形如 `/nexscope:start`。
...
| # | 问题 | 建议 |
|---|---|---|
| Q-5 | `/nexscope:stop` 后 inbox 里未处理的消息如何处理? | 建议保留,下次 start 后仍在 inbox;超过 24h 归 expired |
| Q-6 | v1 是否存消息历史? | 不存。新加入者不看到过去消息 |
| Q-10 | `/nexscope:accept` 后 Claude 完成任务是否自动回一条 `userAgent`? | 建议自动回,简要结果摘要到同 thread |
