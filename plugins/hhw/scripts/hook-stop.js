...
请为每条用 \`/hhw:say --thread=<threadId> @<from> <你的回复>\` 发送回复;回复后 daemon 会自动把该 thread 的 hopCount+1 并清空 pending。
如果你认为不需要回复(例如只是告知),可用 \`/hhw:mode manual\` 暂时退出 auto,或直接再次结束(5 分钟后任务自动降级到 inbox)。`;

  blockWith(reason);
}

main().catch((e) => {
  process.stderr.write(`[hhw hook-stop] ${e.message}\n`);
  allow();
});
