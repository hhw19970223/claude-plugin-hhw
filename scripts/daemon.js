...
relay.on('reconnecting', ({ nextInMs, code }) => {
  pushNotification('system', { text: `[hhw] relay disconnected (code=${code}), reconnecting in ${Math.round(nextInMs/1000)}s` });
});
  ...
  if (state.mode === 'auto' && t.autoReplyCount < state.hopLimit) {
    ...
    if (state.mode === 'auto' && t.autoReplyCount >= state.hopLimit) {
      pushNotification('system', {
        text: `[hhw] thread ${msg.threadId} reached hop limit (${state.hopLimit}); latest mention fell through to inbox. Use /hhw:inbox to handle, or /hhw:mode manual to quiet auto globally.`,
      });
    }
  }
}
...
function onError(msg) {
  ...
  pushNotification('system', { text: `[hhw] relay error: ${msg.code}${msg.message ? ' — ' + msg.message : ''}` });
...
function handleBinaryFrame(chunk) {
  ...
  if (entry.received > entry.declaredSize) {
    ...
    pushNotification('system', { text: `[hhw] incoming file ${entry.attachment.name} from ${entry.from} oversized; dropped.` });
    return;
  }
...
function onFileEnd(msg) {
  ...
  entry.stream.end(() => {
    ...
    if (!ok) {
      ...
      pushNotification('system', { text: `[hhw] file ${entry.attachment.name} from ${entry.from}: size mismatch (got ${entry.received}, expected ${entry.declaredSize}); discarded.` });
      return;
    }
...
async function handleIpc(req) {
  ...
  switch (op) {
    ...
    case 'mode': {
      ...
      writeSession();
      pushNotification('system', { text: `[hhw] mode set to ${state.mode}` });
    ...
    case 'say': {
      ...
      if (isAutoReply && ack.threadId) {
        ...
        if (t.autoReplyCount >= state.hopLimit) {
          pushNotification('system', {
            text: `[hhw] thread ${ack.threadId} reached hop limit (${state.hopLimit}); auto-reply disabled on this thread. New @mentions on it will go to inbox.`,
          });
        }
      }
