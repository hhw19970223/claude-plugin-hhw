...
function formatLine(n) {
  switch (n.kind) {
    case 'presence': {
      if (n.event === 'snapshot') return `[nexscope presence] online: ${(n.users || []).join(', ')}`;
      if (n.event === 'joined')   return `[nexscope presence] ${n.user} joined | online: ${(n.users || []).join(', ')}`;
      if (n.event === 'left')     return `[nexscope presence] ${n.user} left | online: ${(n.users || []).join(', ')}`;
      return `[nexscope presence] (unknown event) ${JSON.stringify(n)}`;
    }
    case 'msg': {
    ...
    case 'system':
      return n.text;
    default:
      return `[nexscope] ${JSON.stringify(n)}`;
  }
}

async function main() {
  const session = readJsonOrNull(SESSION_PATH);
  // If no session (nexscope not started), stay silent — the hook just outputs nothing.
  ...
  try {
    ...
    process.stderr.write(`[nexscope hook-pre-prompt] ${e.message}\n`);
    process.stdout.write(JSON.stringify({}));
    return;
  }
  if (!rows.length) {
  ...
  const header = `[nexscope inbox-stream] ${rows.length} event(s) since last turn (user=${session.name}, mode=${session.mode}):`;
